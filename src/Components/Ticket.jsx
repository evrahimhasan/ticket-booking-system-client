...
const Ticket = () => {
    ...
    useEffect(() => {
        fetch(`https://ticket-booking-system-server.vercel.app/tickets?from=${from || ""}&to=${to || ""}&date=${date || ""}`)
            .then(res => res.json())
    ...
    return (
        <div className="container mx-auto px-4 py-10 space-y-8">
            {tickets.length === 0 ? (
                <div className="max-w-2xl mx-auto text-center bg-white dark:bg-zinc-900 p-12 rounded-[2rem] border border-dashed border-slate-200 dark:border-zinc-800 shadow-sm">

                    <div className="text-5xl mb-6 text-orange-500">
                        <FaBus />
                    </div>

                    <h2 className="text-2xl font-black text-slate-800 dark:text-white uppercase italic tracking-tight mb-4">
                        No Tickets Found
                    </h2>

                    <p className="text-slate-500 dark:text-zinc-400 font-medium mb-6">
                        Sorry! No buses available for
                        <span className="font-bold text-orange-500"> {from} → {to} </span>
                        on <span className="font-bold">{date}</span>.
                    </p>

                    <button
                        onClick={() => navigate('/')}
                        className="px-8 py-4 bg-orange-500 text-white font-black rounded-2xl shadow-lg shadow-orange-500/20 uppercase italic tracking-widest text-xs"
                    >
                        Search Again
                    </button>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex gap-2">
                                        <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider flex items-center gap-1.5 ${ticket.busType === 'AC' ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-500'}`}>
                                            <FaSnowflake className={ticket.busType?.trim().toUpperCase() === 'AC' ? 'animate-spin-slow' : 'opacity-30'} />
                                            {ticket.busType?.trim().toUpperCase() === 'AC' ? 'Premium AC' : 'Non AC'}
                                        </span>
                                    </div>
